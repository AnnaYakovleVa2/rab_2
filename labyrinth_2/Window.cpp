...
void Window::draw_view() {
	...
	double cx = px + cos(_player->dir()) * sin(_player->tilt());
	double cy = py + sin(_player->dir()) * sin(_player->tilt());
	double cz= pz + cos(_player->tilt());
...
void Window::event(const SDL_Event &e) {
	...
	case SDL_MOUSEMOTION:
		if (e.motion.xrel >= 0)
			_player->turn_right(e.motion.xrel * MOUSE_SENSITIVITY);
		else
			_player->turn_left(-e.motion.xrel * MOUSE_SENSITIVITY);
		if (e.motion.yrel >= 0)
			_player->tilt_down(e.motion.yrel * MOUSE_SENSITIVITY);
		else
			_player->tilt_up(-e.motion.yrel * MOUSE_SENSITIVITY);
		break;
...
void Window::update() {
	...
	if (_controls.tilt_up)     _player->tilt_up(TURN_ANGLE);
	if (_controls.tilt_down)   _player->tilt_down(TURN_ANGLE);
